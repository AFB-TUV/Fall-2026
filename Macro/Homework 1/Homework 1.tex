...
\begin{document}
	...
	As expected, the data shows that the separation rate spikes at the start of recessions, while the job-finding rate drops over the course of the recession and then recovers. The job-finding rate should be what drives the slow recoveries. One caveat, though: in this data, the separation rate appears to show a similar pattern to the job-finding rate, though less strongly. Since the year 2000, the imputed job-finding rate has noticeably dropped from its level in the 1900s; the mean appears to be about 0.05 to 0.1 lower.

	The imputed job-finding rate notably misses movements into and out of nonparticipation -- in particular, missing movements from employment to nonparticipation (due to, say, retirement) and movements from nonparticipation into unemployment (as a hedge against increased economic uncertainty). Whether this leads to the job-finding rate being overstated or understated depends on the specifics of the data: if \(N \rightarrow U\) movements are folded into \(u_{t+1}^s\) because they are new entrants into the unemployment pool, the job-finding rate will be overestimated during recessions, but if they are folded into \(u_{t+1}\) because those workers have not held a job for some period before entering the labor force, the job-finding rate will be underestimated during recessions.
	...
	Using this equation, we can compute the steady-state. It is summarized like so:
	...
	\begin{tabular}{|c|c|c|c|}
		...
		& \( w^*  \) & \( \lambda \) & \( u^* \) \\
		\hline
		\(\sigma = 0\) & \( 0.891 \) & \(0.109\) & \(0.0\) \\
		\hline
		\(\sigma = 0.02\) & \( 0.823 \) & \(0.177\) & \(0.101\) \\
		\hline
		\(\sigma = 0.05\) & \( 0.764 \) & \(0.236\) & \(0.175\) \\
		\hline
	\end{tabular}
	\end{center}

	Now we see what happens when we force \( \lambda = \bar{s} = 0.40444  \). This immediately gives \(w^* = 1 - 0.40444 = 0.59556\). Assuming \(\sigma = 0.02\), we can rework the model to be
	\[ b = -C*[w^* - 1)^2 - w^*]     \] 
	Calculating through, we see that this model requires \(b = -2.12\) to achieve the job-finding rate in the data, which is obviously nonsensical. This suggests that the McCall model is not very good at explaining the job-finding rate. The DMP model will modify this by making the job-finding rate dependent on a job-creation decision, which should make the job-finding rate less tied to a very low \(b\).
	
	\subsection{The wage equation, in and out of steady state}

	\subsubsection{}

	We will start by assuming that all of \(w\), \(b\), \(z\), \(u\), \(v\), \(\theta\), and \(\kappa\) could vary to get a more general model. I will adopt the convention of letting a variable \(x_t\) be \(x\) in the current period and \(x'\) in the next in the derivation.
	
	Matches between firms and workers are created by a Cobb-Douglass matching technology:
	...
	\[ S(w) = \Big(W(w') - U(b')\Big) + \Big(J(w') - V(\kappa')\Big)     \]
	...
	The above derivation intentionally avoided imposing the steady-state condition when deriving the equations for the surplus and wage equation; thus, it should be evident that they will hold even if the economy is not in steady state provided the economy is in equilibrium. Because the free-entry condition pins down \(J(w')\) and the other value functions can be represented as transformations of \(J(w')\), the eventual formula ends up not depending on \(\theta\) in later periods, letting wages and the surplus only depend on one period's values.

	\subsubsection{}

	With wages fixed until separation, wages are no longer easy to solve for, as they now depend on the \emph{entire path} of the economy rather than just the next period's bargaining problem. For instance, if the worker and firm believe that the overall productivity of firms \(z\) will increase over the length of a match, then the negotiated wage should be higher initially since it cannot rise when the value of the joint surplus increases with \(z\) later. 

	Mathematically, the Nash bargaining protocol remains the same on the surface; the worker and firm are still splitting the joint surplus. What changes with a lack of wage negotiation is actually the value functions themselves. We still have
	\[ S = \Big(W(w') - U(b')\Big) + \Big(J(w') - V(\kappa')\Big)  \]
	However, if were to write out the Bellman equations, we now have
\begin{align*}
	U(b) &= b + \beta \left[\lambda_w(\theta) W(w') + (1 - \lambda_w(\theta)) U(b')\right]\\
	W(w) &= w + \beta \left[(1 - \sigma) W(w) + \sigma U(b')\right]\\
	V(\kappa) &= - \kappa + \beta \left[\lambda_f(\theta) J(w') + (1 - \lambda_f(\theta)) V(\kappa')\right]\\
	J(w) &= z - w + \beta \left[(1-\sigma) J(w) + \sigma V(\kappa')\right]
	\end{align*}
	In the original setup, the solution to the bargaining problem is pinned down by the free entry condition, which lets us define \(J(w')\) by the free entry condition and substitute it when \(J(w')\) appears in the firm's part of the surplus. However, with \(w\) not able to be renegotiated each period, this substitution is no longer easy. \(V\) still depends on \(J(w')\), since when a vacancy is filled the wage will be negotiated in the next period, but \(J(w)\) now depends not on \(J(w')\) but itself depends on \(J(w)\)! Thus we cannot simply substitute \(J(w')\) to solve the system anymore; we would need some other relationship between \(J(w)\) and \(J(w')\) to solve for the surplus the way we did before.

	\section{Computation}

	\subsection{Calibrate to your hazards}
	
	The earlier derivation was convenient given the previous information, but not particularly useful for comparative statics since we derived it assuming \(\theta^* = 1\). However, it's easy to rederive a more useful formula for comparative statics. Imposing the steady-state, the firm's value function is
	\[ J = w - z + \beta (1 - \sigma) J \implies w = z + (1 - \beta (1 - \sigma)) J    \]
	Substituting the free-entry condition \(  J = \frac{\kappa}{\beta \lambda_f(\theta)} \), we have
	\[ w = z + \frac{\kappa}{\beta \lambda_f(\theta)} - \frac{\kappa (1 - \sigma)}{\lambda_f(\theta)}   \]
	and then substituting \( \beta = \frac{1}{1+r}\) we get
	\[ w = z + \frac{(1 + r - 1 + \sigma) \kappa}{\lambda_f(\theta)} \implies w = z - \frac{(r + \sigma) \kappa}{\lambda_f(\theta)}   \]
	Coupled with the wage equation
	\[ w = (1 - \gamma) b + \gamma (z + \kappa \theta)    \]
	we end up with a system that is easy to solve for equilibrium. Setting these two equations equal and substituting \(\lambda_f(\theta) = \chi \theta^{-\eta}\), we have
	\[ z - \frac{(r + \sigma) \kappa}{\chi \theta^{-\eta}} = (1 - \gamma) b + \gamma (z + \kappa \theta)       \]
	This equation cannot be solved for \(\theta\) analytically in most cases, so we will solve it by bisection. However, we will solve for \(\kappa\) in closed-form in the special case where \(\theta^* = 1\). This gives us 
	\[ z - \frac{(r + \sigma) \kappa}{\chi} = (1 - \gamma) b + \gamma z + \gamma \kappa       \]
	Rearranging, we have
	\[   (1 - \gamma) (z - b) = \left( \gamma + \frac{r + \sigma}{\chi} \right) \kappa \implies \kappa = \frac{(1 - \gamma)(z - b)}{\gamma + \frac{r + \sigma}{\chi}} \]
	This is sufficient to solve for \(\kappa\), which comes out to about \(0.53\). Now we can solve for the steady-state:

	\begin{center}
	\begin{tabular}{|c|c|c|c|c|c|c|c|}
		\hline
		\(\lambda_w\) & \(\lambda_f\) & \(w^*\) & \(u^*\) & \(S\) & \(J\) & \(\frac{1}{\lambda_w}\) & \(\frac{1}{\lambda_f}\) \\
		\hline
		\(0.404\) & \(0.404\) & \(0.964\) & \(0.055\) & \(2.624\) & \(1.312\) & \(2.473\) & \(2.473\)\\
		\hline
	\end{tabular}
	\end{center}

	The model's simulated \(u^*\) is about \(5.5\%\), very close to the estimated \(5.6\%\) in the data. Theoretically there could be some discrepancies because the model does not account for people entering or leaving the labor force, but quantitatively these differences appear to be small in steady-state under this calibration.

	\subsection{Comparative statics}

	We now conduct several comparative statics experiments.

	\begin{center}
	\begin{tabular}{|c|c|c|c|c|c|}
		\hline
		& \(\theta\) & \(\lambda_w\) & \(u^*\) & \(w^*\) & \(\epsilon_{\theta z}\) \\
		\hline
		\(z = 0.98\) & \(0.965\) & \(0.397\) & \(0.055\) & \(0.945\) & \(1.799\) \\
		\hline
		\(z = 1.02\) & \(1.035\) & \(0.412\) & \(0.056\) & \(0.984\) & \(1.748\) \\
		\hline
		\(b = 0.7\) & \(0.474\) & \(0.279\) & \(0.077\) & \(0.975\) & \\
		\hline
		\(\sigma = 0.0955\) & \(0.718\) & \(0.343\) & \(0.055\) & \(0.890\) & \\
		\hline
	\end{tabular}
	\end{center}
	We also examine what happens to unemployment when \(b\) changes:

	Intuitively, \(u\) goes up as \(b\) increases because the value of unemployment goes up, meaning that people are less willing to take jobs. We see that the elasticity increases to very large levels as the financial component of the match \(z-b\) shrinks to zero: intuitively, when the match surplus is very small, the parameters that depend on it are very responsive to even small changes in the surplus. Accordingly, \(\epsilon_{\theta z} = 20\) only when \(b\) is about \(0.94\), at which the match surplus \( z - b = 1.0 - 0.94 = 0.6\) has shrunk to almost nothing.

	\subsection{The Beveridge loop}

	We simulate a \(12\)-period \(2\%\) productivity shock. The response of unemployment and vacancies is shown here:

	\includegraphics[scale=0.25]{mit_uv}
	\includegraphics[scale=0.25]{mit_vacancies}

	Because the shock starts at \(t = 0\), the level of vacancies has already jumped to its higher level. We then see unemployment drop until it reaches the new steady-state, then rise back to the original equilibrium once the shock reverts. This behavior becomes more apparent if we extend our time interval to include \(t = -1\), the period right before the shock. The \emph{direction} of the movements is less clear, but the looping back to the starting position is obvious:
	
	\includegraphics[scale=0.25]{mit_uv_full}
	\includegraphics[scale=0.25]{mit_vacancies_full}

	Vacancies jump when the shock hits, then move sightly in the opposite direction from the jump over the next few periods as the market tightness adjusts through movements of workers into or out of unemployment. It takes about two and a half months for unemployment to close half the gap. 

	\includegraphics[scale=0.5]{mit_diffs}

	What this model is missing is the separations spike at the beginning of the recession, which means that the change in job-finding rates is forced to do all the work in this basic model. 
